...
module Make (Theme : Config.Theme.S) : S = struct
  ...
  let stylize_gemini (type a) ~ctx:{ Context.current_url; history } = function
    ...
    | Header (H1, h) -> Lwt.return @@ LTerm_text.stylise h Theme.h1
    | Header (H2, h) -> Lwt.return @@ LTerm_text.stylise h Theme.h2
    | Header (H3, h) -> Lwt.return @@ LTerm_text.stylise h Theme.h3
    | ListItem item ->
        Lwt.return
        @@ LTerm_text.eval
             [
